...
combined_plot_b_count <- ggplot() +
  ...
  scale_color_manual(values = c("B.N" = "darkgoldenrod1",
                               "W.N" = "black",
                               "B.P" = "brown2",
                               "W.P" = "cornflowerblue", "B" = "black", "W" = "grey"),
...
combined_plot_b_count2 <- ggplot() +
  ...
  geom_jitter(data = brighton, 
             aes(x = Sampling_Date, y = Barnacle_Count, color = Combination), size = 1, alpha = 0.35, width = 2) +
  scale_color_manual(values = c("B.N" = "orange",
                               "W.N" = "grey",
                               "B.P" = "brown2",
                               "W.P" = "forestgreen", "B" = "black", "W" = "grey"),
                     labels = c( "W.N" = "Non-Polluted + Control","B.N" = "Non-Polluted + Warming",  "W.P" = "Polluted + Control", "B.P" = "Polluted + Warming")) +
  scale_shape_manual(values = c("N" = 16, "P" = 17),
                     labels = c("N" = "Non-Polluted Region", "P" = "Polluted Region")) +
  labs(title = "Average Barnacle Count Over Time",
       x = "Sampling Date",
...
grazer_se <- brighton %>%
  group_by(Sampling_Date, Combination) %>%
  summarize( Avg = mean(total_grazer, na.rm = TRUE),
    SE = sd(total_grazer, na.rm = TRUE) / sqrt(n()),  # Calculate SEM based on the total_grazer directly
    Lower = Avg - SE, 
    Upper = Avg + SE,
    .groups = 'drop')
...
combined_plot_grazer <- ggplot() +
  ...
  labs(title = "Grazer Counts Over Time",
       ...
       y = "Grazer Count") +
  theme_minimal()+
  scale_y_continuous("Total Grazer count", limits = c(0, 7))
...
# change order of boxes !!!!!!!! --> like on 1st boxplot
...
# Create a new column for the combined factor with specified levels
sub_JUN_1$treatment_group <- interaction(sub_JUN_1$Pad_Colour, sub_JUN_1$Pad_Region, sep = " + ")

# Set the factor levels to control the order
sub_JUN_1$treatment_group <- factor(sub_JUN_1$treatment_group, levels = c("W + N", "B + N", "W + P", "B + P"))

# Now plot
BP_Jun_1_BC <- ggplot(sub_JUN_1, aes(x = treatment_group, y = Barnacle_Count, fill = treatment_group)) +
  geom_boxplot() +
  ...
  scale_x_discrete(labels = c("W + N" = "Non-Polluted + Control",
                              "B + N" = "Non-Polluted + Warming",
                              "W + P" = "Polluted + Control",
                              "B + P" = "Polluted + Warming")) + 
  scale_fill_manual(values = c("W + N" = "cornflowerblue",
                               "B + N" = "orange",
                               "W + P" = "forestgreen",
                               "B + P" = "brown2"))
...
BP_Jun_1_BS <- ggplot(sub_JUN_1, aes(x = interaction(Pad_Colour, Pad_Region, sep = " + "), y = AVG_Banacle_Size, fill = interaction(Pad_Colour, Pad_Region, sep = " + "))) +
  geom_boxplot() +
  ...
  scale_fill_manual(values = c("B + N" = "orange",
                               "W + N" = "cornflowerblue",
                               "B + P" = "brown2",
                               "W + P" = "forestgreen"))
...
#Jun 1
  ...
  BP_Jun_1_D <- ggplot(sub_JUN_1, aes(x = interaction(Pad_Colour, Pad_Region, sep = " + "), y = Diatoms, , fill = interaction(Pad_Colour, Pad_Region, sep = " + "))) +
  geom_boxplot() +
...
#Jun 1
  ...
  BP_Jun_1_GA <- ggplot(sub_JUN_1, aes(x = interaction(Pad_Colour, Pad_Region, sep = " + "), y = Green_Algae, , fill = interaction(Pad_Colour, Pad_Region, sep = " + "))) +
  geom_boxplot() +
...
#Jun 1
  ...
  BP_Jun_1_C <- ggplot(sub_JUN_1, aes(x = interaction(Pad_Colour, Pad_Region, sep = " + "), y = Cyanobacteria, fill = interaction(Pad_Colour, Pad_Region, sep = " + "))) +
  geom_boxplot() +
...
```{r}
BP_Jun_1_GA <- ggplot(sub_JUN_1, aes(x = interaction(Pad_Colour, Pad_Region, sep = " + "), y = total_grazer, fill = interaction(Pad_Colour, Pad_Region, sep = " + "))) +
  geom_boxplot() +
...
```
```{r}
# variation between treatment plots

brighton$treatment <- with(brighton, paste0(Pad_Region, "_", Pad_Colour))

diatom_data <- brighton %>%
  dplyr::select(treatment, Diatoms)

mean_Diatoms <- diatom_data %>%
  group_by(treatment) %>%
  summarise(mean_diatoms = mean(Diatoms, na.rm = TRUE),
            sd = sd(Diatoms, na.rm = TRUE),
            n = n()) %>%
  mutate(se = sd / sqrt(n))

# Assuming you want to find differences from a baseline treatment, set the baseline
baseline_value <- mean_Diatoms %>%
  filter(treatment == "N_W") %>% # Replace YourBaselineTreatment with your actual baseline
  pull(mean_diatoms)
...
mean_Diatoms$difference_from_baseline <- mean_Diatoms$mean_diatoms - baseline_value

mean_Diatoms$treatment <- factor(mean_Diatoms$treatment, levels = c("N_W", "N_B", "P_W", "P_B"))




diatoms_bar <- ggplot(mean_Diatoms, aes(x = treatment, y = difference_from_baseline, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.5, color = "black", size = 1) + # Using size for thicker outline
  geom_errorbar(aes(ymin = difference_from_baseline - se, ymax = difference_from_baseline + se), width = 0.2, position = position_dodge(0.5)) + # Adjust error bars
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Difference from Baseline (N_W) in Diatom Measurements",
       x = "Treatment",
       y = "Difference from Baseline") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  scale_fill_manual(values = c("N_B" = "orange",
                               "N_W" = "blue",
                               "P_B" = "hotpink",
                               "P_W" = "green" ))
...
Cyanobacteria_data <- brighton %>%
  dplyr::select(treatment, Cyanobacteria)

mean_Cyanobacteria <- Cyanobacteria_data %>%
  group_by(treatment) %>%
  summarise(mean_cyanobacteria = mean(Cyanobacteria, na.rm = TRUE),
            sd = sd(Cyanobacteria, na.rm = TRUE),
            n = n()) %>%
  mutate(se = sd / sqrt(n))

# Assuming you want to find differences from a baseline treatment, set the baseline
baseline_value <- mean_Cyanobacteria %>%
  filter(treatment == "N_W") %>% # Replace YourBaselineTreatment with your actual baseline
  pull(mean_cyanobacteria)




# Calculate differences from the baseline
mean_Cyanobacteria$difference_from_baseline <- mean_Cyanobacteria$mean_cyanobacteria - baseline_value

mean_Cyanobacteria$treatment <- factor(mean_Cyanobacteria$treatment, levels = c("N_W", "N_B", "P_W", "P_B"))




cyanobacteria_bar <- ggplot(mean_Cyanobacteria, aes(x = treatment, y = difference_from_baseline, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.5, color = "black", size = 1) + # Using size for thicker outline
  geom_errorbar(aes(ymin = difference_from_baseline - se, ymax = difference_from_baseline + se), width = 0.2, position = position_dodge(0.5)) + # Adjust error bars
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Difference from Baseline (N_W) in Cyanobacteria Measurements",
       x = "Treatment",
       y = "Difference from Baseline") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
GA_data <- brighton %>%
  dplyr::select(treatment, Green_Algae)

mean_Green_Algae <- GA_data %>%
  group_by(treatment) %>%
  summarise(mean_GA = mean(Green_Algae, na.rm = TRUE),
            sd = sd(Green_Algae, na.rm = TRUE),
            n = n()) %>%
  mutate(se = sd / sqrt(n))

# Assuming you want to find differences from a baseline treatment, set the baseline
baseline_value <- mean_Green_Algae %>%
  filter(treatment == "N_W") %>% # Replace YourBaselineTreatment with your actual baseline
  pull(mean_GA)
...
# Calculate differences from the baseline
mean_Green_Algae$difference_from_baseline <- mean_Green_Algae$mean_GA - baseline_value

mean_Green_Algae$treatment <- factor(mean_Green_Algae$treatment, levels = c("N_W", "N_B", "P_W", "P_B"))



Green_Algae_bar <- ggplot(mean_Green_Algae, aes(x = treatment, y = difference_from_baseline, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.5, color = "black", size = 1) + # Using size for thicker outline
  geom_errorbar(aes(ymin = difference_from_baseline - se, ymax = difference_from_baseline + se), width = 0.2, position = position_dodge(0.5)) + # Adjust error bars
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Difference from Baseline (N_W) in Green Algae Measurements",
       x = "Treatment",
       y = "Difference from Baseline") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
BC_data <- brighton %>%
  dplyr::select(treatment, Barnacle_Count)

mean_BC <- BC_data %>%
  group_by(treatment) %>%
  summarise(mean_bc = mean(Barnacle_Count, na.rm = TRUE),
            sd = sd(Barnacle_Count, na.rm = TRUE),
            n = n(),
...
# Calculate differences from the baseline
mean_BC$difference_from_baseline <- mean_BC$mean_bc - baseline_value

mean_BC$treatment <- factor(mean_BC$treatment, levels = c("N_W", "N_B", "P_W", "P_B"))




BC_bar <- ggplot(mean_BC, aes(x = treatment, y = difference_from_baseline, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.5, color = "black", size = 1) + # Using size for thicker outline
  geom_errorbar(aes(ymin = difference_from_baseline - se, ymax = difference_from_baseline + se), width = 0.2, position = position_dodge(0.5)) + # Adjust error bars
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Difference from Baseline (N_W) in Barnacle Count Measurements",
       x = "Treatment",
       y = "Difference from Baseline") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
BS_data <- brighton %>%
  dplyr::select(treatment, AVG_Banacle_Size)

mean_BS <- BS_data %>%
  group_by(treatment) %>%
  summarise(mean_bs = mean(AVG_Banacle_Size, na.rm = TRUE),
            sd = sd(AVG_Banacle_Size, na.rm = TRUE),
            n = n(),
            .groups = 'drop') %>% # This ensures the result is not grouped
  mutate(se = sd / sqrt(n))
...
# Calculate differences from the baseline
mean_BS$difference_from_baseline <- mean_BS$mean_bs - baseline_value

mean_BS$treatment <- factor(mean_BS$treatment, levels = c("N_W", "N_B", "P_W", "P_B"))




BS_bar <- ggplot(mean_BS, aes(x = treatment, y = difference_from_baseline, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.5, color = "black", size = 1) + # Using size for thicker outline
  geom_errorbar(aes(ymin = difference_from_baseline - se, ymax = difference_from_baseline + se), width = 0.2, position = position_dodge(0.5)) + # Adjust error bars
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Difference from Baseline (N_W) in Barnacle Size Measurements",
       x = "Treatment",
       y = "Difference from Baseline") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
G_data <- brighton %>%
  dplyr::select(treatment, total_grazer)

mean_G <- G_data %>%
  group_by(treatment) %>%
  summarise(mean_g = mean(total_grazer, na.rm = TRUE),
            sd = sd(total_grazer, na.rm = TRUE),
            n = n(),
            .groups = 'drop') %>% # This ensures the result is not grouped
  mutate(se = sd / sqrt(n))
...
# Calculate differences from the baseline
mean_G$difference_from_baseline <- mean_G$mean_g - baseline_value

mean_G$treatment <- factor(mean_G$treatment, levels = c("N_W", "N_B", "P_W", "P_B"))




G_bar <- ggplot(mean_G, aes(x = treatment, y = difference_from_baseline, fill = treatment)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.5, color = "black", size = 1) + # Using size for thicker outline
  geom_errorbar(aes(ymin = difference_from_baseline - se, ymax = difference_from_baseline + se), width = 0.2, position = position_dodge(0.5)) + # Adjust error bars
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(title = "Difference from Baseline (N_W) in Grazer Count Measurements",
       x = "Treatment",
       y = "Difference from Baseline") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
